...
\begin{itemize}
    \item \textit{existent\_resource} state: when sending the delete request, the server can return a 404 error, if the resource exists, or can return a 200 OK.
    \item \textit{non\_existent\_resource} state: when sending the put request, the server can return a 404 error, if the resource does not exist, or can return a 200 OK.
\end{itemize}
\phantom{}\\
